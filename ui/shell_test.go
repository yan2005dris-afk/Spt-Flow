...
func TestMenu_SelectTUIOnly(t *testing.T) {
	m := Model{
		ViewState:          "menu",
		SelectedMenuOption: 0, // "Open TUI only"
		Width:              80,
		Height:             24,
	}

	// Press Enter - should emit MenuChoiceMsg with "tui-only"
	...
	if menuMsg, ok := msg.(MenuChoiceMsg); ok {
		if menuMsg.Choice != ChoiceTUIOnly {
			t.Errorf("Expected MenuChoiceMsg.Choice to be 'tui-only', got %q", menuMsg.Choice)
		}
	} else {
